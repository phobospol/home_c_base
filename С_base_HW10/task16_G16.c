...
void replace_all (char *str, const char *oldstr, const char *newstr)
{
    ...
    // При помощи цикла while bщем в строке str все подстроки "Ling"
    while ((pos = strstr (str, oldstr)) != NULL)
    {
        // Копируем начало строки до найденной подстроки "Ling" в buffer
        strncpy (buffer, str, pos - str);
        buffer [pos - str] = '\0';

        // Добавляем новую подстроку "Cao" в buffer
...
int main(int argc, char **argv)
{
    ...
    // Заменяем все подстроки "ling" на "Cao"
    replace_all (sentence, "Ling", "Cao");
...
// Функция для замены всех вхождений "Ling" на "Cao" в строке
void replace_all_Ling_to_Cao(char *str, size_t max_len)
{
    char buffer[MAX_LENGTH];
    ...
    {
        // Если встречаем последовательность "Ling"
        if (str[i] == 'L' && str[i+1] == 'i' && str[i+2] == 'n'&& str[i+3] == 'g')
        {
           ...
           // buffer[j++] = 'g';
            ...
            i += 4; // пропускаем заменяемые символы
        } else {
            buffer[j++] = str[i++];
        }
    }
...
int main() {
    ...
    while (fgets(sentence, sizeof(sentence), inputfile)) 
    {
        ...
        // Пример: заменяем Ling на Cao
        replace_all_safe(sentence, MAX_LENGTH, "Ling", "Cao");
